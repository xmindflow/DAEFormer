...
class EfficientTransformerBlock(nn.Module):
    ...
    def __init__(self, in_dim, key_dim, value_dim, head_count=1, token_mlp='mix'):
        ...
        # self.norm5 = nn.LayerNorm(int(in_dim*2))
        self.channel_attn = ChannelAttention(in_dim)
        self.norm5 = nn.LayerNorm(2*in_dim)
        self.norm6 = nn.LayerNorm(in_dim)
        self.mlp_reduce = nn.Linear(int(in_dim*2), in_dim)
        # add channel attention here
        if token_mlp=='mix':
            #self.mlp_reduce = MixFFN(int(in_dim*2), int(in_dim*4))
            self.mlp1 = MixFFN(in_dim, int(in_dim*4))
            self.mlp2 = MixFFN(in_dim, int(in_dim*4))
            self.mlp3 = MixFFN(in_dim, int(in_dim*4))
        elif token_mlp=='mix_skip':
            #self.mlp_reduce = MixFFN_skip(int(in_dim*2), int(in_dim*4))
            self.mlp1 = MixFFN_skip(in_dim, int(in_dim*4))
            self.mlp2 = MixFFN_skip(in_dim, int(in_dim*4))
            self.mlp3 = MixFFN_skip(in_dim, int(in_dim*4))
        else:
            #self.mlp_Reduce = MLP_FFN(int(in_dim*2), int(in_dim*4))
            self.mlp1 = MLP_FFN(in_dim, int(in_dim*4))
            self.mlp2 = MLP_FFN(in_dim, int(in_dim*4))
            self.mlp3 = MLP_FFN(in_dim, int(in_dim*4))

    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        # dual attention, fuse by concatenation
        norm1 = self.norm1(x)
        ...
        attn_norm = self.norm3(attn)
        attn_mlp = self.mlp1(attn_norm, H, W)
        attn_mlp = attn + attn_mlp

        norm_channel = self.norm2(x)
        attn_channel = self.channel_attn(norm_channel)
        attn_channel = attn_channel + x

        channel_norm = self.norm4(attn_channel)
        channel_mlp = self.mlp2(channel_norm, H, W)
        channel_mlp = channel_mlp + channel_norm

        attn_cat = torch.cat((attn_mlp, channel_mlp), dim=2)
        cat_norm = self.norm5(attn_cat)
        cat_mlp = self.mlp_reduce(cat_norm)

        norm6 = self.norm6(cat_mlp)
        mlp3 = self.mlp3(norm6, H, W)
        mx = mlp3 + cat_mlp
        return mx
...
class ChannelFuseFormerCat(nn.Module):
