...
class EfficientTransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        # dual attention, fusion by adding after norm and ffn
        norm1 = self.norm1(x)
        norm1 = Rearrange('b (h w) d -> b d h w', h=H, w=W)(norm1)
        
        spatial_attn = self.attn(norm1)
        spatial_attn = Rearrange('b d h w -> b (h w) d')(spatial_attn)
        
        add_spatial = spatial_attn + x
        norm3 = self.norm3(add_spatial)
        mlp1 = self.mlp1(norm3, H, W)
        add_spatial_2 = mlp1 + add_spatial

        norm2 = self.norm2(x)
        channel_attn = self.channel_attn(norm2)

        add_channel = channel_attn + x
        norm4 = self.norm4(add_channel)
        mlp2 = self.mlp2(norm4, H, W)
        add_channel_2 = mlp2 + add_channel

        fuse_add = add_channel_2 + add_spatial_2
        norm5 = self.norm5(fuse_add)
        mlp3 = self.mlp3(norm5, H, W)
        mx = mlp3 + fuse_add
        return mx
...
class ChannelFuseFormerAdd2(nn.Module):
