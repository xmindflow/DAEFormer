...
class EfficientTransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        # dual attention parallely, then add channel and spatial
        norm1 = self.norm1(x)
        ...
        fuse_add = attn + channel_attn
        fuse_norm = self.norm3(fuse_add)
        fuse_mlp = self.mlp1(fuse_norm, H, W)
        mx = fuse_add + fuse_mlp
        return mx
...
class ChannelFuseFormerAdd1(nn.Module):
