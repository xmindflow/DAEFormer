...
class MISSFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        out_enc = self.backbone(x)
        # bridge = self.bridge(encoder) #list

        b,c,_,_ = out_enc[3].shape
        # print(bridge[3].shape, bridge[2].shape,bridge[1].shape, bridge[0].shape)
        #---------------Decoder-------------------------     
        # print("stage3-----")   
        tmp_3 = self.decoder_3(out_enc[3].permute(0,2,3,1).view(b,-1,c))
        # print("stage2-----")   
        tmp_2 = self.decoder_2(tmp_3, out_enc[2].permute(0,2,3,1))
        # print("stage1-----")   
        tmp_1 = self.decoder_1(tmp_2, out_enc[1].permute(0,2,3,1))
        # print("stage0-----")  
        tmp_0 = self.decoder_0(tmp_1, out_enc[0].permute(0,2,3,1))
