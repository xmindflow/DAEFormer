...
class ChannelEffFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=4, token_mlp_mode="mix_skip"):
        super().__init__()
    
        # Encoder
        dims, key_dim, value_dim, layers = [[64, 128, 320, 512], [64, 128, 320, 512], [64, 128, 320, 512], [2, 2, 2, 2]]        
        self.backbone = MiT(image_size=128, in_dim=dims, key_dim=key_dim, value_dim=value_dim, layers=layers,
                            head_count=head_count, token_mlp=token_mlp_mode)
        
        # Decoder
        d_base_feat_size = 4 #16 for 512 input size, and 7 for 224
