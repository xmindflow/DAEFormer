...
class MyDecoderLayer(nn.Module):
    ...
    def forward(self, x1, x2=None):
        if x2 is not None:# skip connection exist
            ...
            if self.last_layer:
                out = self.last_layer(self.layer_up(tran_layer_4).view(b, 4*h, 4*w, -1).permute(0,3,1,2)) 
            else:
                out = self.layer_up(tran_layer_4)
        else:
            # if len(x1.shape)>3:
...
class ChannelEffFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        ...
        # Encoder
        dims, key_dim, value_dim, layers = [[64, 128, 320, 512], [64, 128, 320, 512], [64, 128, 320, 512], [4, 4, 4, 4]]        
