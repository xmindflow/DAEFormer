...
class MyDecoderLayer(nn.Module):
    def __init__(self, input_size, in_out_chan, head_count, token_mlp_mode, n_class=9,
        ...
        self.layer_former_3 = EfficientTransformerBlock(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
        self.layer_former_4 = EfficientTransformerBlock(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
        self.layer_former_5 = EfficientTransformerBlock(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
        self.layer_former_6 = EfficientTransformerBlock(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
        self.layer_former_7 = EfficientTransformerBlock(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
        self.layer_former_8 = EfficientTransformerBlock(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
    ...
    def forward(self, x1, x2=None):
        if x2 is not None:# skip connection exist
            ...
            tran_layer_3 = self.layer_former_3(tran_layer_2, h, w)
            tran_layer_4 = self.layer_former_4(tran_layer_3, h, w)
            tran_layer_5 = self.layer_former_5(tran_layer_4, h, w)
            tran_layer_6 = self.layer_former_6(tran_layer_5, h, w)
            tran_layer_7 = self.layer_former_7(tran_layer_6, h, w)
            tran_layer_8 = self.layer_former_8(tran_layer_7, h, w)
            
            
            if self.last_layer:
                out = self.last_layer(self.layer_up(tran_layer_8).view(b, 4*h, 4*w, -1).permute(0,3,1,2)) 
            else:
                out = self.layer_up(tran_layer_8)
        else:
            # if len(x1.shape)>3:
...
class ChannelEffFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        ...
        # Encoder
        dims, key_dim, value_dim, layers = [[64, 128, 320, 512], [64, 128, 320, 512], [64, 128, 320, 512], [8, 8, 8, 8]]        
