...
if args.model_name == 'swin_res50_cv_120_221_66_true_cfg' and args.dataset == 'ISIC17':
   args.save_name =  args.output_dir + f'/{args.dataset.lower()}/swin_res50_cv_120_221_66_true_cfg_0.9401161108906095_ep_129.model'
   print(f'loaded: {args.save_name}')
elif args.model_name == 'swin_res50_cv_120_221_66_true_cfg' and args.dataset == 'ISIC18':
    args.save_name =  args.output_dir + f'/{args.dataset.lower()}/swin_res50_cv_120_221_66_true_cfg_0.9247337204608267_ep_88.model'
    print(f'loaded: {args.save_name}')
##########
if __name__ == "__main__":
    ...
    ker_sz = []
    th = []
    print("DO NOT TOUCH ME! I'M SEARCHING ...")
    for thresh in tqdm(np.arange (0.3, 0.5, 0.01), desc=f"Predicting", position=0): #np.arange (0.3, 0.5, 0.01)
        for mk in tqdm(range(1, 9), desc = f"Threshold = {thresh}", position=1, leave=False): #range(1, 9)
            th.append(thresh)
            ker_sz.append(mk)
            predictions = []
            gt = []
...
#                 print('val_mode')
                ...
                tqdm_loop = tqdm(enumerate(test_loader),
                                 ...
                                 position=3, leave=False, desc=f"Kernel size = {mk}")
...
#             print("\n\n")
    ...
    print('\n')
    print(f'Now EXPLORE THE [ enhanced_results_{args.model_name}_{args.dataset}_ep_129.csv ]')
    print('\n \n')